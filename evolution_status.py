...
- 同一データでの検証日数: **{state.get('data_day_index', '確認できません')}**
- Champion: **{state.get('champion_version', '確認できません')}**
- 本日の研究Winner: **{state.get('research_winner', '確認できません')}**
- 本日のChampion昇格: **{promoted}**
- 同一データSHAでの追加昇格ロック: **{locked}**
- 累積Challenger評価数: **{state.get('total_evaluations', 0)}**
- 累積Champion昇格数: **{state.get('total_promotions', 0)}**
- 本日の候補あたり有意水準: **{state.get('per_candidate_alpha', '確認できません')}**
- 進化履歴ハッシュ: `{state.get('last_history_hash', '')}`

> 日次ではChallenger世代を生成・バックテストします。同じ抽せんデータに対する反復検定はalpha-spendingで厳格化し、Championは同一データSHAにつき最大1回だけ昇格できます。
"""
