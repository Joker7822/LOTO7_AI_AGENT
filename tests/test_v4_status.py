...
def test_strict_evidence_lines_show_paired_metrics():
    ...
    assert "+0.1000 / +0.0500" in text
    assert "62.5% / 75.0%" in text
...
def test_strict_evidence_lines_do_not_mislabel_legacy_only_evidence():
    ...
    assert "まだstrict採点なし" in text


def test_holdout_lines_render_progress_and_both_references():
    holdout = {
        "status": "active",
