...
.toolbar{margin:18px 0 12px}.toolbar input{width:100%;color:var(--text);background:#0d1d2c;border:1px solid var(--line);border-radius:14px;padding:13px 14px;outline:none}.toolbar input:focus{border-color:rgba(90,167,255,.7);box-shadow:0 0 0 3px rgba(90,167,255,.11)}
...
<main class="wrap">
  ...
  <?php else: ?>
    <div class="toolbar"><input id="searchBox" type="search" placeholder="予測番号・モデルを検索…" autocomplete="off"></div>
    <section class="panel">
      <?php if (count($rows) === 0): ?>
      ...
      <?php else: ?>
        <div class="table-wrap">
          <table>
            ...
            <tbody>
            <?php foreach ($rows as $row): $searchText=mb_strtolower(implode(' ',array($row['predicted_numbers'],$row['model_version'])),'UTF-8'); ?>
              <tr class="filter-row" data-search="<?= h($searchText) ?>">
        ...
        <div class="mobile-list">
          <?php foreach ($rows as $row): $searchText=mb_strtolower(implode(' ',array($row['predicted_numbers'],$row['model_version'])),'UTF-8'); ?>
            <article class="card filter-row" data-search="<?= h($searchText) ?>">
...
<script>
(function(){const search=document.getElementById('searchBox');if(!search)return;search.addEventListener('input',function(){const q=search.value.trim().toLowerCase();document.querySelectorAll('.filter-row').forEach(function(row){row.classList.toggle('hide',!!q&&!(row.dataset.search||'').includes(q));});});})();
</script>
</body>
</html>
