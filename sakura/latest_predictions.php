...
try {
    ...
    $roundStmt = $pdo->query('SELECT MAX(' . $roundExpr . ') AS latest_round FROM loto7_predictions');
...
WHERE CAST(REPLACE(REPLACE(target_round, '第', ''), '回', '') AS UNSIGNED) = :latest_round
ORDER BY ticket ASC, prediction_created_at_jst DESC
SQL
        );
        $stmt->execute(array(':latest_round' => $latestRound));
        $rows = $stmt->fetchAll();
    }
...
<style>
:root{--bg:#07111f;--panel:#0d1b2a;--line:rgba(255,255,255,.09);--text:#eef6ff;--muted:#8ca3ba;--cyan:#55e6d7;--green:#55d98d;--red:#ff7081;--shadow:0 24px 70px rgba(0,0,0,.28)}
...
<main class="wrap">
  <section class="hero">
    ...
    <h1>LOTO7 最新予測</h1>
    <p class="subtitle">loto7_predictions の最新対象回のみ表示</p>
    <div class="stats">
      <div class="stat"><div class="stat-label">最新対象回</div><div class="stat-value"><?= $latestRound !== null ? '第' . h($latestRound) . '回' : '—' ?></div></div>
      <div class="stat"><div class="stat-label">レコード数</div><div class="stat-value"><?= h($totalCount) ?></div></div>
  ...
  <?php else: ?>
    <div class="toolbar">
      <div class="search"><input id="searchBox" type="search" placeholder="予測番号・モデルを検索…" autocomplete="off"></div>
      ...
      <?php else: ?>
        <div class="table-wrap">
          <table>
            ...
            <tbody>
            <?php foreach ($rows as $row): $active=(int)$row['is_active']===1; $searchText=mb_strtolower(implode(' ',array($row['predicted_numbers'],$row['model_version'])),'UTF-8'); ?>
              <tr class="filter-row" data-status="<?= $active?'active':'inactive' ?>" data-search="<?= h($searchText) ?>">
        ...
        <div class="mobile-list">
          <?php foreach ($rows as $row): $active=(int)$row['is_active']===1; $searchText=mb_strtolower(implode(' ',array($row['predicted_numbers'],$row['model_version'])),'UTF-8'); ?>
            <article class="card filter-row" data-status="<?= $active?'active':'inactive' ?>" data-search="<?= h($searchText) ?>">
...
<script>
(function(){const search=document.getElementById('searchBox'),status=document.getElementById('statusFilter');if(!search||!status)return;function apply(){const q=search.value.trim().toLowerCase(),s=status.value;document.querySelectorAll('.filter-row').forEach(function(row){const okQ=!q||(row.dataset.search||'').includes(q),okS=s==='all'||row.dataset.status===s;row.classList.toggle('hide',!(okQ&&okS));});}search.addEventListener('input',apply);status.addEventListener('change',apply);})();
</script>
</body>
</html>
